...
export abstract class AbstractCollectionPage {
  ...
  getTableRow(selector: number | string) {
    return typeof selector === 'string' ? this.getTableRowByCode(selector) : this.getTable.getByRole('row').nth(selector)
  }

  getTableRowByCode(code: string) {
    const _code = new RegExp(`^${code}$`)
    return this.getTable.getByRole('row').filter(
      { has: this.page.locator(`td`, {hasText: _code})}
    )
  }

  getNavigationLink(rowSelector: number|string, linkType: keyof typeof this.navigationLinksTypes) {
    return this.getTableRow(rowSelector).getByRole('link').nth(this.navigationLinksTypes[linkType])
  }
